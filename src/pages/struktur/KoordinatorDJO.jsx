    ...
    <div className="font-helvetica-regular bg-primary-white pb-20 relative overflow-hidden">
      ...
      <section className="mx-auto  lg:px-24 container flex justify-center gap-1 lg:gap-5 py-10 lg:flex-nowrap flex-wrap">
        {DJO[0].kementerian.map((item, index) => (
            ...
            desc={item.desc}
            image={item.image}
            length={DJO[0].kementerian.length}
            index={index}
      ...
      <div
        onClick={() => moveToTop()}
        className={`${isActive ? "visible opacity-100" : "invisible opacity-0"} duration-300 ease-in-out w-10 lg:w-14 flex justify-center items-center border-2  aspect-square rounded-full bg-primary-tealBlue fixed bottom-5 lg:bottom-10 right-5 lg:right-10`}
      >
        <Icon icon={"fluent:arrow-up-12-filled"} className="text-2xl text-primary-white"/>
      </div>
    </div>
  );
};
