...
const CardProker = ({ index, nama, desc }) => {
    return (
        <div
            ...
            } w-full aspect-[12/14] rounded-xl p-7 hover:-translate-y-1 duration-300 ease-in-out hover:shadow-xl group`}
        >
            <p className="font-helvetica-extraBold text-4xl text-end">
                {index + 1}
            </p>
            <p className="font-helvetica-extraBold text-xl">{nama}</p>
            <p className="text-sm group-hover:text-base duration-200 ease-in-out">{desc}</p>
        </div>
    );
};
